...
def create_user(usernew: schemas.users.UserCreate, db: Session = Depends(get_db)):
    db_user = crud.users.get_user(db, id=id)
    if db_user:
        raise HTTPException(status_code=400, detail="Usuario existente intenta nuevamente")
    return crud.users.create_user(db=db, user=usernew)
